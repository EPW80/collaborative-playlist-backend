  ...
  async getTrackInfo(artist, track) {
    try {
      ...
      return {
        ...
        duration: trackInfo.duration ? Math.floor(trackInfo.duration / 1000) : null,
        playcount: trackInfo.playcount,
        listeners: trackInfo.listeners,
        url: trackInfo.url,
        image: trackInfo.album?.image?.find(img => img.size === 'medium')?.['#text'] || null,
  ...
  async getArtistInfo(artist) {
    try {
      ...
      return {
        ...
        url: artistInfo.url,
        image: artistInfo.image?.find(img => img.size === 'medium')?.['#text'] || null,

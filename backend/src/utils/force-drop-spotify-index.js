...
async function forceDropSpotifyIndex() {
  try {
    console.log("🔧 Force dropping spotifyId index from production database...");

    // Connect to MongoDB (use the production URI from environment)
    const uri = process.env.MONGODB_URI || "mongodb://localhost:27017/collaborative-playlist";
    ...
    indexes.forEach((index, i) => {
      console.log(`  ${i + 1}. ${index.name}: ${JSON.stringify(index.key)} ${index.unique ? '(UNIQUE)' : ''} ${index.sparse ? '(SPARSE)' : ''}`);
    });

    // Try to drop any spotifyId-related indexes
    const spotifyIndexes = indexes.filter(idx => 
    ...
    if (spotifyIndexes.length === 0) {
      ...
      console.log(`🎯 Found ${spotifyIndexes.length} spotifyId index(es) to remove:`);
      
      for (const index of spotifyIndexes) {
        try {
          console.log(`   Dropping: ${index.name} ${JSON.stringify(index.key)}`);
